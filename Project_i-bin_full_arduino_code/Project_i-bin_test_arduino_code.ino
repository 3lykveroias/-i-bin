...
int green = A2;            //Έξοδος πράσινου LED
int yellow = A3;           //Έξοδος κίτρινου LED
int red = A4;              //Έξοδος κόκκινου LED
...
void setup() {
  ...
  lidservo.attach(servopin);  // Προσάρτηση (attah) του servo κινητήρα στο pin 11, στο αντικείμενο servo 
}
...
void loop() {
  ...
  // Ο αισθητήρας στέλνει ένα σήμα με τη μορή ήχου υψηλής συχνότητας. Μέσω του trig pin αποστέλεται ένας HIGH παλμός 10 ή περισσοτέρωων δευτερολέπτων.
...
//--------------------------Τέλος δοκιμαστικών εντολών--------------------------
  ...
  if (send_time==0){
    ...
    if (send_time>=150){ //αποστολή δεδομένων ανα 5 λεπτά: 150 φορές ανα 2 δευτερόλεπτα = 300 δευτερόλεπτα, δηλαδή 5 λεπτά
      Serial.println(httpRequestData);
      delay(500);
      send_time=0;
    }
