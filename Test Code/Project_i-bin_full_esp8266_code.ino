/**
 * @file Project_i-bin_full_esp8266_code.ino
 * @brief Test code for the i-bin project, created for the Open Technologies Contest (ELAK)
 * @details This code sets up an ESP8266 microcontroller to connect to a WiFi network and send data to a MySQL database via a PHP script. It includes libraries for working with the ESP8266 and the Wire.h library for working with I2C devices. The code defines the WiFi network credentials, domain name, and URL path for the PHP script. It also defines an API key for security purposes. The setup() function initializes the serial communication and connects to the WiFi network. The loop() function checks the WiFi connection status and sends data to the database using an HTTP POST request.
 * @date 2019-2020
 */
