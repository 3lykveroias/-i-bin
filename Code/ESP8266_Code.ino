...
/**
 * @file ESP8266_Code.ino
 * @brief Sends data from ESP8266 to a PHP file which inserts the data into a MySQL database.
 * 
 * This code sends data from ESP8266 to a PHP file which inserts the data into a MySQL database.
 * It uses the ESP8266WiFi and ESP8266HTTPClient libraries for WiFi and HTTP communication.
 * The WiFi network credentials and the server name and URL path to the PHP file are defined in the code.
 * The code also defines an API key for security reasons and the sensor name and location.
 * The loop function reads data from the serial port and sends it to the PHP file using HTTP POST request.
 * The HTTP response code is printed to the serial monitor for debugging purposes.
 */
...
const char *ssid = " όνομα_SSID_δικτύου";
const char *password = "κωδικός_πρόσβασης_δικτύου";
...
// Ορισμός ενός API key για λόγους ασφαλείας στη μεταφόρτωση των δεδομένων.
